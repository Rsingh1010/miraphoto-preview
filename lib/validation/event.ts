...
// NOTE: by design, this intentionally duplicates the same organization
// fields already collected on the Organization Profile page
// (lib/validation/organization.ts). Both pages are being kept as full,
// independent forms for review purposes. Reusing organizationSchema.shape
// here instead of re-typing each field avoids duplicating the validation
// rules themselves, even though the fields appear on both pages.
const eventOnlySchema = z.object({
  title: z.string().trim().min(5, "Title must be at least 5 characters"),

  eventDate: z.string().min(1, "Event date is required"),

  startTime: z.string().regex(TIME_24H_REGEX, "Enter a valid start time"),
